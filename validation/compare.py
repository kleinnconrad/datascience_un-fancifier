...
def main():
    ...
    actual_neg_lr = cm_lr[0][0] + cm_lr[0][1] # Total Actual Negatives
    actual_pos_lr = cm_lr[1][0] + cm_lr[1][1] # Total Actual Positives
    ...
    print("   Confusion Matrix (Absolute & % of Actual Class):")
    print(f"      [TN: {cm_lr[0][0]:4d} ({cm_lr[0][0]/actual_neg_lr*100:4.1f}%) | FP: {cm_lr[0][1]:4d} ({cm_lr[0][1]/actual_neg_lr*100:4.1f}%)]")
    print(f"      [FN: {cm_lr[1][0]:4d} ({cm_lr[1][0]/actual_pos_lr*100:4.1f}%) | TP: {cm_lr[1][1]:4d} ({cm_lr[1][1]/actual_pos_lr*100:4.1f}%)]\n")
    
    # Evaluate Extracted Rules
    cm_rules = confusion_matrix(y, rule_predictions)
    actual_neg_rules = cm_rules[0][0] + cm_rules[0][1]
    actual_pos_rules = cm_rules[1][0] + cm_rules[1][1]
    ...
    print("   Confusion Matrix (Absolute & % of Actual Class):")
    print(f"      [TN: {cm_rules[0][0]:4d} ({cm_rules[0][0]/actual_neg_rules*100:4.1f}%) | FP: {cm_rules[0][1]:4d} ({cm_rules[0][1]/actual_neg_rules*100:4.1f}%)]")
    print(f"      [FN: {cm_rules[1][0]:4d} ({cm_rules[1][0]/actual_pos_rules*100:4.1f}%) | TP: {cm_rules[1][1]:4d} ({cm_rules[1][1]/actual_pos_rules*100:4.1f}%)]")
    print("======================================================")
